...
static void inc_prioritized_task_count(struct rq *rq, struct task_struct *p)
{
	...
	if (schedtune_prefer_idle(p) && p->prio <= DEFAULT_PRIO) {
		if (likely(!bitset)) {
...
cpu_is_in_target_set(struct task_struct *p, int cpu)
{
	...
	int first_cpu = (schedtune_task_boost(p) && p->prio <= DEFAULT_PRIO) ?
	...
	*backup_cpu = -1;
	...
	do {
		...
		if ((prefer_idle && best_idle_cpu != -1) ||
			...
			(boosted && p->prio > DEFAULT_PRIO && target_cpu != -1)) {
			if (prioritized_task) {
				/*
