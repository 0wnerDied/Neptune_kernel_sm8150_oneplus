...
module_param(asm330lhhx_delay_gyro, int, 0644);
	...
	while (read_len < fifo_len) {
		...
		for (i = 0; i < word_len; i += ST_ASM330LHHX_FIFO_SAMPLE_SIZE) {
			...
			if (tag == ST_ASM330LHHX_TS_TAG) {
				...
				if (!test_bit(ST_ASM330LHHX_HW_FLUSH,
							&hw->state))
...
static int st_asm330lhhx_update_fifo(struct iio_dev *iio_dev, bool enable)
{
	...
	if (sensor->id == ST_ASM330LHHX_ID_TEMP) {
		if (!(hw->enable_mask & (BIT(ST_ASM330LHHX_ID_ACC) |
			...
			hw->odr_table_entry[
			ST_ASM330LHHX_ID_ACC].batching_reg.addr,
			hw->odr_table_entry[
			ST_ASM330LHHX_ID_ACC].batching_reg.mask,
			data);
			if (err < 0)
				goto out;
		}
