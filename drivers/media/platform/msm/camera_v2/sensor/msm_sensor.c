	...
	void *argp)
{
	...
	case CFG_SET_STEREO_CFG: {
		...
		for (i = 0; i < MAX_CAMERAS; i++) {
			...
			if (p_ctrl && p_ctrl->sensordata && p_ctrl != s_ctrl &&
				!strcmp(p_ctrl->sensordata->sensor_name,
				s_ctrl->sensordata->sensor_name)) {
				...
				p_ctrl->sensordata->cam_slave_info
					->is_stereo_config = 1;
	...
	case CFG_SET_STEREO_AEC: {
		...
		CDBG("peer ctrl %p stereo config %d stereo master %d",
		...
		if (s_ctrl->peer_sensor_ctrl &&
			s_ctrl->sensordata->cam_slave_info->is_stereo_config &&
			!s_ctrl->sensordata->cam_slave_info->stereo_aux) {
			rc = s_ctrl->peer_sensor_ctrl->sensor_i2c_client
				->i2c_func_tbl->i2c_write_table(
					s_ctrl->peer_sensor_ctrl
					->sensor_i2c_client,
					&conf_array);
